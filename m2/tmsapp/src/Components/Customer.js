...
export default function Profile({ userRole }) {
  ...
  const handleBookNow = (tourNumber) => {
    // You can get the selected tour number from the form input
    // and then redirect to the PaymentPage with the tourNumber parameter
    navigate(`/customer/${tourNumber}/payment`);
  }

  // RECAPTCHA
  // State to manage reCAPTCHA verification
  ...
  // Conditionally enable the "POST" button
  const isPostButtonDisabled = !recaptchaVerified;

  // disabling buttons for customer & hotelier
      ...
      <Paper elevation={6}  sx={{height:'160vh' , width:'30%', backgroundColor:'rgba(0, 0, 0, 0.4) !important'}} style={{marginBottom:20}}>
        ...
        {userRole === 'admin' && (
          ...
          <Paper sx={{backgroundColor:'#64CCC5', m:3}}>
            ...
            />
            <Button sx={{width:'90%'}} style={{marginTop: 10, marginLeft:10, marginBottom:10}} variant='contained' onClick={handleUpload} disabled={isPostButtonDisabled}>Upload</Button>
          ...
          <Paper sx={{backgroundColor:'rgba(0, 0, 0, 0.4) !important', m:3}}>
            ...
            <Card sx={{backgroundColor:'#64CCC5'}}>
              ...
              <Stack direction="column" spacing={3} sx={{m:3}}>
                ...
                onChange={handleRecaptchaChange}
              />
                <Button variant='contained' onClick={() => handleBookNow(tourNumber)} disabled={isPostButtonDisabled}><Link to={`/customer/${tourNumber}/payment`}>PAY NOW</Link></Button>
                <Button variant='contained' onClick={handleSubmit} disabled={isPostButtonDisabled}>BOOK NOW</Button>
                  ...
                  {item._id === editingCustomer._id ? (
                    ...
                    <>
                      ...
                      <TextField
                        ...
                        label="Tour Number"
                        value={editingCustomer.tourNumber || item.tourNumber}
                        InputLabelProps={{ shrink: true }}
                        sx={{ m: 1 }}
                      />
                      <TextField
